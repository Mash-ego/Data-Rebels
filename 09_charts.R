...
COL <- c(Declining="#C0392B", Stable="#7F8C8D", Growing="#27AE60")
...
tx <- read_csv("transactional_banking.csv",
               col_types = cols(date=col_date("%Y-%m-%d"), memo=col_character(), reference=col_character(), .default=col_guess()),
...
cat("=== DECK GRAPHICS ===\n")
# Every plot is assigned p1..p13 so it can be referenced by number in the deck
# and re-rendered individually without re-running the whole script.
...
p4 <- tx %>% filter(leg_type != "intercompany_sweeps") %>%
  ...
  labs(title="Why the quarterly comparison had to be like-for-like",
       ...
       caption="Consumer clients peak at around 37% above average in Q4. Comparing H1 to the preceding H2 subtracts a Christmas peak from a January trough.") +
...
p5 <- ggplot(mix, aes(reorder(leg_type, value), value, fill=name)) +
...
p6 <- snap %>% left_join(hmm %>% select(entity_id, current_state), by="entity_id") %>%
...
p7 <- snap %>% left_join(hmm %>% select(entity_id, current_state), by="entity_id") %>%
...
p8 <- snap %>% ggplot(aes(reorder(entity_name, sweep_share), sweep_share, fill=sector)) +
  geom_col() + coord_flip() +
...
# --- 09 ENSEMBLE AGREEMENT: three models, one verdict 
# Uses vote_hmm / vote_trend / vote_yoy. The earlier sig_hmm / sig_trend /
# sig_yoy columns no longer exist: they counted DECLINE signals only, which
# scored a client growing 19% as "0/3" -- correct arithmetic that reads as the
# models disagreeing. The ensemble now votes on DIRECTION.
if (all(c("vote_hmm","vote_trend","vote_yoy") %in% names(hmm))) {
  p9 <- hmm %>%
      select(entity_name, agreement,
             HMM = vote_hmm, `36-month trend` = vote_trend, `Year-on-year` = vote_yoy) %>%
      pivot_longer(-c(entity_name, agreement), names_to = "model", values_to = "vote") %>%
      mutate(entity_name = reorder(entity_name, agreement),
             model = factor(model, levels = c("HMM","36-month trend","Year-on-year"))) %>%
      ggplot(aes(model, entity_name, fill = vote)) +
      geom_tile(colour = "white", linewidth = .8) +
      geom_text(aes(label = substr(vote, 1, 1)), size = 3.2,
                colour = "white", fontface = "bold") +
      scale_fill_manual(values = COL, name = NULL) +
      labs(title = "Three models, one verdict",
           subtitle = "Each method votes on direction; the ensemble takes the majority",
           x = NULL, y = NULL,
           caption = "D = Declining, S = Stable, G = Growing. Ordered by agreement. A 1-1-1 split is reported as Split, not forced to a winner.") +
      thm
  sv(p9, "09_ensemble_agreement.png", 9, 7)
} else {
  cat("  SKIPPED 09_ensemble_agreement -- vote_* columns missing; re-run the pipeline\n")
}
...
if (!is.null(fc)) {
  ...
  p10 <- fcw %>% mutate(entity_name=factor(entity_name, levels=ordf)) %>%
  ...
  sv(p10, "10_forecast_decline_risk.png", 11, 8)

  p11 <- fc %>% filter(horizon_months==12) %>%
  ...
  sv(p11, "11_rands_at_risk.png", 11, 7)
}
...
if (!is.null(clu) && "peer_group" %in% names(clu)) {
  ...
  sv(p12, "12_peer_vs_cluster.png", 9, 6)
}

# --- 13 opportunity heatmap 
if (!is.null(pill)) {
  p13 <- pill %>% group_by(entity_id) %>% filter(fy==max(fy)) %>% ungroup() %>%
  ...
  sv(p13, "13_opportunity_heatmap.png", 10, 8)
}
